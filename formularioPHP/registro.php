<?php
    ...
    $edad = $_POST['edad'];     // me pregunto si habrá post.get para manejar los null sin que explote
    ...
    if (mysqli_num_rows($ya_existe) > 0) {
        ...
        // esto es horrible, ¿no hay una mejor forma de hacerlo?
        $insert_value = "INSERT INTO `$db_name`.`$db_table_name` (`Nombre` , `Edad`) VALUES (" . '"'. $nombre . '", ' . $edad . ')';  

        mysqli_select_db($db_connection, $db_name);
        $retry_value = mysqli_query($db_connection, $insert_value); // pa ver si sizas o nozas

        if (!$retry_value) {
            die('Error: ' . mysqli_error($db_connection));
        }
            
        // me pregunto si hay forma de mandarle las cosas registradas al front
        // ¿tendré que hacerle un post a algo tipo exito.php?
